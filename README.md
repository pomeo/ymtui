...
- 🌊 **Моя волна** — бесконечное персональное радио с подгрузкой и подстройкой,
  а также волны **под занятие / настроение / жанр / эпоху** (Бег, Просыпаюсь,
  Грустное…) — выберите «Моя волна» и загляните в нижний блок
...
- 🖼 **Обложка альбома** в плеере (опционально, включается в `Ctrl+P`)
- 🎛 **MPRIS**: управление мультимедиа-клавишами и через `playerctl` (с обложкой)
...
### 4. (Опционально) Обложка альбома

Показ обложки в плеере (включается в `Ctrl+P`). Лучше всего смотрится в
терминалах с графикой (kitty, WezTerm, iTerm2, foot), в остальных — фолбэк:

```bash
pipx install "ymtui[cover]"     # можно совмещать: ymtui[mpris,cover]
```
